...
3. **Aesthetics Matter**: Code formatting, variable clarity, and interface ergonomics reflect how deeply you care about the humans who will read your work tomorrow.
  ...
  `}function B(){const i=document.getElementById("hud-clock");if(i){const l=()=>{const O=new Date().toLocaleTimeString("en-US",{hour12:!1,timeZone:"Asia/Kolkata"});i.innerText=`${O} IST`};l(),setInterval(l,1e3)}const e=document.getElementById("sound-toggle-btn"),t=document.getElementById("sound-state-label");e&&t&&e.addEventListener("click",()=>{const l=m.toggle();t.innerText=l?"[ AUDIO: ON ]":"[ AUDIO: OFF ]",l&&m.playClick()});const n=document.getElementById("theme-toggle-btn"),a=document.getElementById("theme-state-label"),s=l=>{l?(document.documentElement.classList.remove("theme-dark"),document.documentElement.classList.add("theme-light"),document.body.classList.remove("theme-dark"),document.body.classList.add("theme-light"),a&&(a.innerText="[ THEME: WHITE ]")):(document.documentElement.classList.remove("theme-light"),document.documentElement.classList.add("theme-dark"),document.body.classList.remove("theme-light"),document.body.classList.add("theme-dark"),a&&(a.innerText="[ THEME: DARK ]"))},r=localStorage.getItem("profile-theme")||"dark";s(r==="light"),n&&n.addEventListener("click",()=>{const b=!document.body.classList.contains("theme-light");s(b),localStorage.setItem("profile-theme",b?"light":"dark"),m.playClick()});const d=document.getElementById("mobile-nav-toggle"),p=document.getElementById("main-nav");d&&p&&(d.addEventListener("click",()=>{p.classList.toggle("open");const l=p.classList.contains("open");d.innerHTML=l?"<span>[ CLOSE ]</span>":"<span>[ MENU ]</span>",m.playClick()}),p.querySelectorAll(".nav-link").forEach(l=>{l.addEventListener("click",()=>{p.classList.remove("open"),d.innerHTML="<span>[ MENU ]</span>"})}));const T=document.getElementById("back-to-top");T&&T.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"}),m.playClick()})}function H(){const i=o.projects.slice(0,2),e=o.articles.slice(0,2);return`
  ...
  `}let v="ALL";function C(){const i=v==="ALL"?o.projects:o.projects.filter(e=>e.category===v);return`
      ...
      <div class="container">
        ...
        <div class="filter-bar">
          ${["ALL","SYSTEMS","CREATIVE TECH"].map(e=>`
            <button class="filter-btn ${v===e?"active":""}" data-category="${e}">
              [ ${e} ]
            </button>
          `).join("")}
        </div>
  ...
  `}function R(i){document.querySelectorAll(".filter-btn").forEach(n=>{n.addEventListener("click",a=>{v=a.target.dataset.category;const s=document.getElementById("view-container");s&&(s.innerHTML=C(),R(i))})}),document.querySelectorAll(".project-item").forEach(n=>{n.addEventListener("click",()=>{const a=n.dataset.projectId,s=o.projects.find(r=>r.id===a);s&&typeof i=="function"&&i("project",s)})})}let f="ALL";function k(){const i=f==="ALL"?o.articles:o.articles.filter(e=>e.category===f);return`
    <div class="view-section">
      <div class="container">
        <!-- Header -->
        ...
        <div class="filter-bar">
          ${["ALL","ENGINEERING","DESIGN","CAREER"].map(e=>`
            <button class="filter-btn blog-filter-btn ${f===e?"active":""}" data-category="${e}">
              [ ${e} ]
            </button>
          `).join("")}
        </div>
          ...
          <div class="contact-statement-wrap">
            ...
            <table class="contact-meta-table">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <a href="${o.contact.github}" target="_blank" rel="noopener" style="color: var(--text-primary); text-decoration: underline; margin-right: 1rem;">GITHUB ↗</a>
                    <a href="${o.contact.linkedin}" target="_blank" rel="noopener" style="color: var(--text-primary); text-decoration: underline; margin-right: 1rem;">LINKEDIN ↗</a>
                    <a href="${o.contact.twitter}" target="_blank" rel="noopener" style="color: var(--text-primary); text-decoration: underline;">X / TWITTER ↗</a>
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <div class="direct-email-box">
              <div>
                ...
                <div style="font-family: var(--font-mono); font-size: 0.95rem; font-weight: 600; color: var(--text-primary);">${o.contact.email}</div>
              </div>
  ...
  `}function j(){const i=document.getElementById("copy-email-btn");i&&i.addEventListener("click",()=>{navigator.clipboard.writeText(o.contact.email).then(()=>{S("EMAIL COPIED TO BUFFER"),m.playClick()})});const e=document.getElementById("transmission-form"),t=document.getElementById("transmission-receipt"),n=document.getElementById("reset-form-btn");e&&t&&(e.addEventListener("submit",a=>{a.preventDefault();const s=document.getElementById("contact-name").value,r=document.getElementById("contact-email").value;m.playTransmit();const d=new Date().toISOString(),p="0x"+Math.random().toString(16).substr(2,10).toUpperCase();document.getElementById("receipt-timestamp").innerText=d,document.getElementById("receipt-hash").innerText=p,document.getElementById("receipt-sender").innerText=`${s} <${r}>`,e.style.display="none",t.style.display="block",S("TRANSMISSION DELIVERED")}),n&&n.addEventListener("click",()=>{e.reset(),t.style.display="none",e.style.display="flex",m.playClick()}))}const x=document.getElementById("site-header"),w=document.getElementById("site-footer"),c=document.getElementById("modal-container"),u=document.getElementById("modal-content"),A=document.getElementById("modal-close-btn"),h=document.getElementById("custom-cursor"),E=document.getElementById("custom-cursor-dot");x&&(x.innerHTML=M());w&&(w.innerHTML=P());B();if(h&&E){let i=window.innerWidth/2,e=window.innerHeight/2,t=i,n=e;window.addEventListener("mousemove",r=>{i=r.clientX,e=r.clientY,E.style.left=`${i}px`,E.style.top=`${e}px`},{passive:!0});const a=()=>{t+=(i-t)*.22,n+=(e-n)*.22,h.style.left=`${t}px`,h.style.top=`${n}px`,requestAnimationFrame(a)};a();const s=()=>{document.querySelectorAll("a, button, .project-item, .article-card, input, textarea, select").forEach(d=>{d.addEventListener("mouseenter",()=>{h.classList.add("hovering")}),d.addEventListener("mouseleave",()=>{h.classList.remove("hovering")})})};window.addEventListener("routechange",()=>{setTimeout(s,50)}),s()}function G(){const i=document.querySelector(".brand-logo");i&&!i._boundScramble&&(i._boundScramble=!0,i.addEventListener("mouseenter",()=>{const e=i.getAttribute("data-text")||"SALMAN",t=i.querySelector("span");t&&L(t,e,25)}))}function g(i,e){if(!c||!u)return;m.playClick(),c.style.display="flex";const t=document.querySelector(".modal-dialog");if(t&&(t.scrollTop=0),setTimeout(()=>c.classList.add("open"),10),document.body.style.overflow="hidden",i==="project")u.innerHTML=`
      <div>
    ...
    `;const a=document.getElementById("modal-reader-close");a&&a.addEventListener("click",y)}const n=u.querySelector(".glitch-text");n&&L(n,n.dataset.text,25)}function y(){c&&(c.classList.remove("open"),document.body.style.overflow="",setTimeout(()=>{c.style.display="none",u&&(u.innerHTML="")},250))}A&&A.addEventListener("click",y);c&&c.addEventListener("click",i=>{i.target===c&&y()});window.addEventListener("keydown",i=>{i.key==="Escape"&&c&&c.classList.contains("open")&&y()});function F(i){return i.split(`

`).map(e=>{if(e=e.trim(),!e)return"";if(e.startsWith("### "))return`<h3 class="modal-section-h3">${e.replace("### ","")}</h3>`;if(e.startsWith("> "))return`<blockquote style="border-left: 2px solid var(--text-primary); padding-left: 1.5rem; margin: 2rem 0; font-style: italic; color: var(--text-primary);">${e.replace("> ","")}</blockquote>`;if(e.startsWith("```")){const t=e.replace(/```[a-z]*\n?/g,"").trim();return`<pre class="code-block-minimal"><code>${W(t)}</code></pre>`}return e.startsWith("1. ")||e.startsWith("- ")?`<ul style="padding-left: 1.5rem; margin-bottom: 1.5rem; line-height: 1.8;">${e.split(`
`).map(n=>`<li>${n.replace(/^(\d+\.|\-)\s+/,"")}</li>`).join("")}</ul>`:`<p>${e}</p>`}).join("")}function W(i){return i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const q={home:()=>H(),projects:()=>C(),blog:()=>k(),contact:()=>U()},z=new $(q);window.addEventListener("routechange",i=>{const e=i.detail.route;if(document.querySelectorAll(".nav-link").forEach(t=>{t.dataset.route===e?t.classList.add("active"):t.classList.remove("active")}),e==="home"){const t=document.querySelector(".hero-wrapper");t&&(t.addEventListener("mouseenter",()=>t.classList.add("hero-hovered")),t.addEventListener("mouseleave",()=>t.classList.remove("hero-hovered"))),document.querySelectorAll(".project-item").forEach(n=>{n.addEventListener("click",()=>{const a=n.dataset.projectId,s=o.projects.find(r=>r.id===a);s&&g("project",s)})}),document.querySelectorAll(".article-card").forEach(n=>{n.addEventListener("click",()=>{const a=n.dataset.articleId,s=o.articles.find(r=>r.id===a);s&&g("article",s)})})}else e==="projects"?R((t,n)=>g(t,n)):e==="blog"?N((t,n)=>g(t,n)):e==="contact"&&j();G()});z.init();
