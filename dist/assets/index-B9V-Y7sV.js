...
3. **Aesthetics Matter**: Code formatting, variable clarity, and interface ergonomics reflect how deeply you care about the humans who will read your work tomorrow.
  ...
  `}function $(){const i=document.getElementById("hud-clock");if(i){const r=()=>{const f=new Date().toLocaleTimeString("en-US",{hour12:!1,timeZone:"Asia/Kolkata"});i.innerText=`${f} IST`};r(),setInterval(r,1e3)}const e=document.getElementById("sound-toggle-btn"),t=document.getElementById("sound-state-label");e&&t&&e.addEventListener("click",()=>{const r=l.toggle();t.innerText=r?"[ AUDIO: ON ]":"[ AUDIO: OFF ]",r&&l.playClick()});const n=document.getElementById("mobile-nav-toggle"),a=document.getElementById("main-nav");n&&a&&(n.addEventListener("click",()=>{a.classList.toggle("open");const r=a.classList.contains("open");n.innerHTML=r?"<span>[ CLOSE ]</span>":"<span>[ MENU ]</span>",l.playClick()}),a.querySelectorAll(".nav-link").forEach(r=>{r.addEventListener("click",()=>{a.classList.remove("open"),n.innerHTML="<span>[ MENU ]</span>"})}));const s=document.getElementById("back-to-top");s&&s.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"}),l.playClick()})}function D(){const i=o.projects.slice(0,2),e=o.articles.slice(0,2);return`
  ...
  `}let h="ALL";function w(){const i=h==="ALL"?o.projects:o.projects.filter(e=>e.category===h);return`
      ...
      <div class="container">
        ...
        <div class="filter-bar">
          ${["ALL","SYSTEMS","CREATIVE TECH"].map(e=>`
            <button class="filter-btn ${h===e?"active":""}" data-category="${e}">
              [ ${e} ]
            </button>
          `).join("")}
        </div>
  ...
  `}function A(i){document.querySelectorAll(".filter-btn").forEach(n=>{n.addEventListener("click",a=>{h=a.target.dataset.category;const s=document.getElementById("view-container");s&&(s.innerHTML=w(),A(i))})}),document.querySelectorAll(".project-item").forEach(n=>{n.addEventListener("click",()=>{const a=n.dataset.projectId,s=o.projects.find(r=>r.id===a);s&&typeof i=="function"&&i("project",s)})})}let g="ALL";function C(){const i=g==="ALL"?o.articles:o.articles.filter(e=>e.category===g);return`
    <div class="view-section">
      <div class="container">
        <!-- Header -->
        ...
        <div class="filter-bar">
          ${["ALL","ENGINEERING","DESIGN","CAREER"].map(e=>`
            <button class="filter-btn blog-filter-btn ${g===e?"active":""}" data-category="${e}">
              [ ${e} ]
            </button>
          `).join("")}
        </div>
  ...
  `}function R(i){document.querySelectorAll(".blog-filter-btn").forEach(n=>{n.addEventListener("click",a=>{g=a.target.dataset.category;const s=document.getElementById("view-container");s&&(s.innerHTML=C(),R(i))})}),document.querySelectorAll(".article-card").forEach(n=>{n.addEventListener("click",()=>{const a=n.dataset.articleId,s=o.articles.find(r=>r.id===a);s&&typeof i=="function"&&i("article",s)})})}function M(){return`
    <div class="view-section">
      <div class="container">
        <div class="projects-header">
  ...
  `}function P(){const i=document.getElementById("copy-email-btn");i&&i.addEventListener("click",()=>{navigator.clipboard.writeText(o.contact.email).then(()=>{E("EMAIL COPIED TO BUFFER"),l.playClick()})});const e=document.getElementById("transmission-form"),t=document.getElementById("transmission-receipt"),n=document.getElementById("reset-form-btn");e&&t&&(e.addEventListener("submit",a=>{a.preventDefault();const s=document.getElementById("contact-name").value,r=document.getElementById("contact-email").value;l.playTransmit();const m=new Date().toISOString(),f="0x"+Math.random().toString(16).substr(2,10).toUpperCase();document.getElementById("receipt-timestamp").innerText=m,document.getElementById("receipt-hash").innerText=f,document.getElementById("receipt-sender").innerText=`${s} <${r}>`,e.style.display="none",t.style.display="block",E("TRANSMISSION DELIVERED")}),n&&n.addEventListener("click",()=>{e.reset(),t.style.display="none",e.style.display="flex",l.playClick()}))}const T=document.getElementById("site-header"),I=document.getElementById("site-footer"),c=document.getElementById("modal-container"),d=document.getElementById("modal-content"),S=document.getElementById("modal-close-btn"),p=document.getElementById("custom-cursor"),y=document.getElementById("custom-cursor-dot");T&&(T.innerHTML=k());I&&(I.innerHTML=O());$();if(p&&y){let i=window.innerWidth/2,e=window.innerHeight/2,t=i,n=e;window.addEventListener("mousemove",r=>{i=r.clientX,e=r.clientY,y.style.left=`${i}px`,y.style.top=`${e}px`},{passive:!0});const a=()=>{t+=(i-t)*.22,n+=(e-n)*.22,p.style.left=`${t}px`,p.style.top=`${n}px`,requestAnimationFrame(a)};a();const s=()=>{document.querySelectorAll("a, button, .project-item, .article-card, input, textarea, select").forEach(m=>{m.addEventListener("mouseenter",()=>{p.classList.add("hovering")}),m.addEventListener("mouseleave",()=>{p.classList.remove("hovering")})})};window.addEventListener("routechange",()=>{setTimeout(s,50)}),s()}function B(){const i=document.querySelector(".brand-logo");i&&!i._boundScramble&&(i._boundScramble=!0,i.addEventListener("mouseenter",()=>{const e=i.getAttribute("data-text")||"SALMAN",t=i.querySelector("span");t&&x(t,e,25)}))}function u(i,e){if(!c||!d)return;l.playClick(),c.style.display="flex";const t=document.querySelector(".modal-dialog");if(t&&(t.scrollTop=0),setTimeout(()=>c.classList.add("open"),10),document.body.style.overflow="hidden",i==="project")d.innerHTML=`
      <div>
    ...
    `;const a=document.getElementById("modal-reader-close");a&&a.addEventListener("click",v)}const n=d.querySelector(".glitch-text");n&&x(n,n.dataset.text,25)}function v(){c&&(c.classList.remove("open"),document.body.style.overflow="",setTimeout(()=>{c.style.display="none",d&&(d.innerHTML="")},250))}S&&S.addEventListener("click",v);c&&c.addEventListener("click",i=>{i.target===c&&v()});window.addEventListener("keydown",i=>{i.key==="Escape"&&c&&c.classList.contains("open")&&v()});function U(i){return i.split(`

`).map(e=>{if(e=e.trim(),!e)return"";if(e.startsWith("### "))return`<h3 class="modal-section-h3">${e.replace("### ","")}</h3>`;if(e.startsWith("> "))return`<blockquote style="border-left: 2px solid #ffffff; padding-left: 1.5rem; margin: 2rem 0; font-style: italic; color: #ffffff;">${e.replace("> ","")}</blockquote>`;if(e.startsWith("```")){const t=e.replace(/```[a-z]*\n?/g,"").trim();return`<pre class="code-block-minimal"><code>${j(t)}</code></pre>`}return e.startsWith("1. ")||e.startsWith("- ")?`<ul style="padding-left: 1.5rem; margin-bottom: 1.5rem; line-height: 1.8;">${e.split(`
`).map(n=>`<li>${n.replace(/^(\d+\.|\-)\s+/,"")}</li>`).join("")}</ul>`:`<p>${e}</p>`}).join("")}function j(i){return i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const H={home:()=>D(),projects:()=>w(),blog:()=>C(),contact:()=>M()},G=new L(H);window.addEventListener("routechange",i=>{const e=i.detail.route;if(document.querySelectorAll(".nav-link").forEach(t=>{t.dataset.route===e?t.classList.add("active"):t.classList.remove("active")}),e==="home"){const t=document.querySelector(".hero-wrapper");t&&(t.addEventListener("mouseenter",()=>t.classList.add("hero-hovered")),t.addEventListener("mouseleave",()=>t.classList.remove("hero-hovered"))),document.querySelectorAll(".project-item").forEach(n=>{n.addEventListener("click",()=>{const a=n.dataset.projectId,s=o.projects.find(r=>r.id===a);s&&u("project",s)})}),document.querySelectorAll(".article-card").forEach(n=>{n.addEventListener("click",()=>{const a=n.dataset.articleId,s=o.articles.find(r=>r.id===a);s&&u("article",s)})})}else e==="projects"?A((t,n)=>u(t,n)):e==="blog"?R((t,n)=>u(t,n)):e==="contact"&&P();B()});G.init();
